...
int CALLBACK wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow) {
	HWND hWnd = JaeCreateWindow(hInstance, L"Voxel Game", L"Voxel Game", 1600, 900);
...
void VoxelGame::Initialize() {
	...
	lightBuffer = shared_ptr<ConstantBuffer>(new ConstantBuffer(sizeof(LightBuffer), "Light CB"));
	...
	shared_ptr<Texture> uvgridTexture = AssetDatabase::GetAsset<Texture>("uvgrid");
	shared_ptr<Texture> barrelTexture = AssetDatabase::GetAsset<Texture>("barrel_combined");
	...
	shared_ptr<Texture> rifleTexture = AssetDatabase::GetAsset<Texture>("rifle_combined");
	...
	OutputDebugf("%s\n", formatToString(barrelNormalTexture->GetTexture()->GetDesc().Format));

	rifleMaterial->SetFloat("roughness", 1.0f);
	rifleMaterial->SetFloat("metallic", 1.0f);
	barrelMaterial->SetFloat("roughness", 1.0f);
	barrelMaterial->SetFloat("metallic", 1.0f);

	defaultMaterial->SetCBuffer("LightBuffer", lightBuffer);
	barrelMaterial->SetCBuffer("LightBuffer", lightBuffer);
	rifleMaterial->SetCBuffer("LightBuffer", lightBuffer);

	barrelMaterial->SetTexture("CombinedTex", barrelTexture);
	barrelMaterial->SetTexture("NormalTex", barrelNormalTexture);
	barrelMaterial->SetTexture("MetallicTex", barrelMetallicTexture);
	rifleMaterial->SetTexture("CombinedTex", rifleTexture);
	...
	g_ScissorRect.top = 0;
	g_ScissorRect.right = window->GetWidth();
	g_ScissorRect.bottom = window->GetHeight();
}
VoxelGame::~VoxelGame() {}
...
#pragma endregion
	...
	lightBuffer->Write(&light, sizeof(LightBuffer));
...
void VoxelGame::Render(shared_ptr<CommandList> commandList, D3D12_CPU_DESCRIPTOR_HANDLE rtv, D3D12_CPU_DESCRIPTOR_HANDLE dsv) {
	D3D12_VIEWPORT vp = CD3DX12_VIEWPORT(0.f, 0.f, (float)Graphics::GetWindow()->GetWidth(), (float)Graphics::GetWindow()->GetHeight());
	commandList->D3DCommandList()->RSSetViewports(1, &vp);
	commandList->D3DCommandList()->RSSetScissorRects(1, &g_ScissorRect);
