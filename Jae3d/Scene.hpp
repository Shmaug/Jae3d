...
public:
	...
	JAE_API void DrawSkybox(const std::shared_ptr<CommandList>& commandList);
	// Draws the scene, according to each renderer's RenderQueue()
	JAE_API void Draw(const std::shared_ptr<CommandList>& commandList, const std::shared_ptr<Camera>& camera, const std::shared_ptr<Material>& materialOverride = nullptr);
	JAE_API void DebugDraw(const std::shared_ptr<CommandList>& commandList, const std::shared_ptr<Camera>& camera);
	JAE_API void CollectLights(const DirectX::BoundingFrustum &frustum, jvector<Light*> &lights);

	JAE_API void Intersect(const DirectX::XMVECTOR& point, jvector<std::shared_ptr<Object>>& result) const;
	JAE_API void Intersect(const DirectX::XMFLOAT3& point, jvector<std::shared_ptr<Object>>& result) const;
	JAE_API void Intersect(const DirectX::BoundingOrientedBox& bounds, jvector<std::shared_ptr<Object>>& result) const;
	JAE_API void Intersect(const DirectX::BoundingSphere& sphere, jvector<std::shared_ptr<Object>>& result) const;
	...
	template<class T>
	bool RemoveObject(const std::shared_ptr<T>& object) {
		static_assert(std::is_base_of<Object, T>::value, "T must be an Object!");
	...
	void Skybox(const std::shared_ptr<Material>& s) { mSkybox = s; }
...
private:
	...
	JAE_API void DebugDrawBox(const std::shared_ptr<CommandList>& commandList, const DirectX::BoundingOrientedBox &box, const DirectX::XMMATRIX &vp);
};
