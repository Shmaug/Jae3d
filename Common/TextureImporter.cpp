...
TextureAsset* TextureImporter::Import(string path) {
	...
	return Convert(*image, &info, GetName(path));
}
TextureAsset* TextureImporter::ImportTGA(string path) {
	auto image = make_unique<ScratchImage>();
	TexMetadata info;
...
TextureAsset* TextureImporter::ImportDDS(string path) {
	auto image = make_unique<ScratchImage>();
	TexMetadata info;
