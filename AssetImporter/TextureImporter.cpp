...
const unordered_map<jwstring, ALPHA_MODE> alphaModes{
...
bool skipspacet(wstring &str, int &i) {
	while (i < str.length() && iswspace(str[i]))
		i++;
	return i >= str.length();
}
bool skipalnumt(wstring &str, int &i) {
	while (i < str.length() && (iswalnum(str[i]) || str[i] == '-' || str[i] == '_' || str[i] == '.'))
		i++;
	return i >= str.length();
}

void ReadMetadata(jwstring imagePath, TexMeta &data) {
	...
	for (int i = 0; i < tags.size(); i++) {
		...
		jwstring u = tags[i].value.upper();
		jwstring l = tags[i].value.lower();
		if (name == L"format") {
			if (formats.count(u))
				data.format = formats.at(u);
			else
				cerr << "Invalid format " << tags[i].value.c_str() << "\n";

		} else if (name == L"mipmaps")
			data.mipMaps = l == L"true";

		else if (name == L"linear")
			data.linear = l == L"true";

		else if (name == L"alpha") {
			if (alphaModes.count(l))
				data.alphaMode = alphaModes.at(l);
			else
				cerr << "Invalid alpha mode " << tags[i].value.c_str() << "\n";
		}
	}
}

Texture* ConvertTexture(unique_ptr<ScratchImage> &image, TexMetadata &info, TexMeta &meta, jwstring name) {
	DirectX::Blob blob;
	...
	return new Texture(
		...
		(unsigned int)info.arraySize, info.format, meta.alphaMode,
		(unsigned int)info.mipLevels, blob.GetBufferPointer(), blob.GetBufferSize(), true);
}

Texture* ProcessImage(unique_ptr<ScratchImage> &image, TexMeta &meta, jwstring name) {
	wprintf(L"%s:\n", name.c_str());
...
Texture* ImportTexture(jwstring path) {
	...
	metadata.alphaMode = ALPHA_MODE_TRANSPARENCY;
	ReadMetadata(path, metadata);

	return ImportTexture(path, metadata);
}
Texture* ImportTexture(jwstring path, TexMeta &metadata) {
	jwstring ext = GetExtW(path).lower();
	TexMetadata info;
